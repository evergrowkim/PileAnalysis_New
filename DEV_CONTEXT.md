...
| v2.4 | dc77e72 | 코드 개선 + 버그 수정 + 신규 기능 (12개 항목) |
| v2.5 | (현재) | UX 개선 7건: 버그 수정 + 가이드 시스템 + 시각화 강화 |

### v2.5 변경사항 상세
1. **침하량 음수 버그 수정**: `ratio_tip` 클램프 [0,1] + Ss/Sp/Sps/St에 Math.max(…,0) 적용
2. **말뚝두부 EL → 말뚝 상단 EL**: 용어 통일 + info(?) 버튼 추가 (GUIDE_DATA.pileTopEL)
3. **OK/NG 배지 툴팁**: 사이드바 OK/NG에 title 속성 추가 (침하량 기준 설명)
4. **지지방법 가이드 팝업**: 암반근입/Meyerhof 선택 시 자동 가이드 모달 표시
5. **기준값 저장/복원**: localStorage 기반 사용자 기준값 저장 + 차이 시 주황 테두리 하이라이트
6. **JSON vs 수동 입력 시각 구분**: `_fromJSON` 플래그 + 파란 배경/보더 + 범례 표시
7. **누적 지지력 분포 차트**: SVG 차트 (주면마찰력/선단지지력/허용지지력 + 지층 경계) — 수직지지력 탭 하단 + 요약 탭 팝업 버튼

**신규 함수**: `renderCumulativeCapacityChart()`, `showCumulativeChartPopup()`, `showGuideModal()`, `handleBearingMethodChange()`, `getUserDefaults()`, `saveUserDefaults()`, `restoreUserDefaults()`
...
- [x] ~~침하량 음수 버그~~ → v2.5에서 ratio_tip 클램프 + Math.max 적용
- [x] ~~말뚝두부 EL 용어 혼란~~ → v2.5에서 "말뚝 상단 EL"로 통일
- [x] ~~OK/NG 배지 의미 불명확~~ → v2.5에서 툴팁 추가
- [x] ~~지지방법 선택 가이드 부재~~ → v2.5에서 팝업 가이드 추가
- [x] ~~기본값 복원이 계수 3개만~~ → v2.5에서 사용자 기준값 저장/복원 체계 구축
- [x] ~~JSON/수동 입력 시각 구분 없음~~ → v2.5에서 파란 배경 + 범례 추가
- [x] ~~누적 지지력 시각화 부족~~ → v2.5에서 SVG 차트 + 팝업 추가
- [ ] AI 채팅에 이미지/차트 첨부 기능
